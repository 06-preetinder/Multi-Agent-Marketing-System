...
def memory_summary():
    """Live counts per memory tier — short_term, long_term, episodic (SQLite)
    plus a semantic edge count from Neo4j if enabled. Powers the memory-tier
    readout in the dashboard."""
    conn = _get_conn()
    cur = conn.cursor()
    cur.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='memory'")
    if not cur.fetchone():
        conn.close()
        return jsonify({"short_term": 0, "long_term": 0, "episodic": 0, "semantic": 0})

    cur.execute("SELECT type, COUNT(*) FROM memory GROUP BY type")
    ...
    if drv is not None:
        try:
            with drv.session() as s:
                rec = s.run("MATCH ()-[r:REL]->() RETURN count(r) AS c").single()
                semantic_count = int(rec["c"]) if rec else 0
        except Exception as e:
            app.logger.warning(f"semantic count failed: {e}")
