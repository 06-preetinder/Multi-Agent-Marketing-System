...
def triage_lead(lead_id: str):
    ...
    ltm_hit = bool(ltm and isinstance(ltm, dict))
    if ltm_hit:
        triage = ltm.get("category", triage)

    # Semantic hints
    semantic_hits = query_semantic_for_lead(lead_id)
    for rel in semantic_hits:
    ...
    ins_st = save_memory("lead_triage", "short_term", {"lead_id": lead_id, "category": triage}, key=lead_id)
    ins_ep = save_memory("lead_triage", "episodic", {"episode": f"triage:{lead_id}", "outcome": triage}, key=lead_id)
    result["_memory"] = {
        "reads": (["long_term"] if ltm_hit else []) + (["semantic"] if semantic_hits else []),
        "writes": [t for t, ins in (("short_term", ins_st), ("episodic", ins_ep)) if ins],
    }
...
def engage_lead(lead_id: str, category: str):
    ...
    # save memories + SEM
    ins_st = save_memory("engagement", "short_term", result, key=lead_id)
    ins_lt = save_memory("engagement", "long_term", {"lead_id": lead_id, "preferences": {"channel": preferred}}, key=lead_id)
    ins_ep = save_memory(
        "engagement",
        "episodic",
    ...
    result["_memory"] = {
        "reads": [],
        "writes": [t for t, ins in (("short_term", ins_st), ("long_term", ins_lt), ("episodic", ins_ep)) if ins] + ["semantic"],
    }
...
def optimize_campaign(campaign_id: str):
    ...
    if daily.empty or metric not in daily.columns:
        ...
        ins_st = save_memory("campaign_opt", "short_term", result, key=campaign_id)
        result["_memory"] = {"reads": [], "writes": (["short_term"] if ins_st else [])}
    ...
    ins_st = save_memory("campaign_opt", "short_term", result, key=campaign_id)
    ins_ep = save_memory("campaign_opt", "episodic", {"episode": f"opt:{campaign_id}", "action_sequence": ["decide"], "decision": decision}, key=campaign_id)
    add_semantic_triple(f"Campaign:{campaign_id}", "optimized_for", result["kpi"])
    result["_memory"] = {
        "reads": [],
        "writes": [t for t, ins in (("short_term", ins_st), ("episodic", ins_ep)) if ins] + ["semantic"],
    }
    _emit("campaign_decided", result)
